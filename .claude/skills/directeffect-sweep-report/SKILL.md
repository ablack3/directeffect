...
description: Run the directeffect surface fit + OHDSI EmpiricalCalibration on a drug-sweep output (target-vs-comparator Cox HRs with negative/positive controls, living under private/), and produce a standardized Excel workbook plus a self-contained HTML report. Use when the user asks to "run directeffect on a sweep," "calibrate and report a sweep," "produce the xlsx/report for <sweep>," or points at a drug-sweep's output data and wants triangulated, calibrated effect estimates written up.
---

# directeffect sweep → calibrated report

This is the repeatable version of a one-off pipeline built by hand for a specific sweep
(see that sweep's own `SWEEP_OUTPUT_SPEC.md`, if it has one, for the original data-format
investigation this skill generalizes, and the `directeffect` skill for the underlying
package API). There is **no package function or CLI for this yet** — every run means
writing/adapting R scripts per this spec. If you're doing this for the third time on the
third sweep, that's the signal to actually build `report_sweep()` into the package
...
never `git add` any of it. Generated report bundles are also often moved out of this repo
entirely once delivered — ask, don't assume, unless already instructed.
...
**Shape A — xlsx + per-arm folder tree**:
...
**Shape B — flat comparisons/controls CSVs** (a flat, grid/pipeline-engine style shape):
...
- `negative_controls.csv` — the negative-control set(s), shared/referenced rather than
  duplicated per-arm.
- No per-arm folder tree; calibration inputs come from wherever the pipeline's calibration
  export lands (check `HANDOFF.md`/`README.md` in that specific sweep directory — this
  shape can change across sweeps from the same source, don't assume a layout you've seen
  before is still current).
...
3. Require a minimum estimable-control count (5 is a reasonable starting point — adjust and
   document if this sweep's controls are sparser/denser) before fitting; skip and flag
   (`calibrated = FALSE`, `skip_reason`) rather than crash on arms below it.
...
dark-theme-aware CSS (see any prior `results.html`/`results_calibrated.html` this skill has
produced as the concrete style reference, if one is available: title + subtitle, caveat
callout boxes, summary stat cards, plots with captions, detail tables, appendix). Section
order, standardized across sweeps:
...
6. Full drug ranking table — raw vs calibrated estimate + CI + significance, flipped rows
   highlighted.
7. Per-drug drill-down section for any drug the user specifically asked about (a
   calibration-before-after table/plot pair for that drug) — optional, only when there's a
   specific drug of interest, not generated for every drug in the sweep by default.
...
9. Appendix — links to any external literature cited in a drill-down section (a
   biological-mechanism write-up is a common template for this), and the exact list of
   source files this report was built from (traceability).
...
- If the sweep is large enough that Cox/PSM computation itself needs (re-)running — as
  opposed to just being read and reported on — that's a different, much bigger job: check
  with the user about compute location (local machine vs a remote/cloud environment) and
  resource headroom before doing that. This skill assumes the sweep's raw
  Cox/PSM/negative-control output already exists on disk and only the reporting/calibration
  layer needs running.
